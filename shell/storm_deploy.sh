...
#
# Storm的Docker部署脚本
# 参数1：cmdb_read storm server_environment
# 参数2：image id
...
server_log_path=/home/kepay/storm/logs
contrainer_log_path=/home/kepay/storm/logs

spring_profiles_active=$(cmdb_read storm server_environment)
...
java_opts=$(cmdb_read storm java_opts)
if [ -z "$java_opts" ]; then
...
current_id=$(docker ps --filter name=storm | grep registry.docker.nigeriainternal.com | awk '{print $2}' | awk -F '-' '{print $NF}')
...
docker pull registry.docker.nigeriainternal.com/kepay/storm:1.0.0-SNAPSHOT-$upgrade_id
if [ $? -ne 0 ]; then
  print_error "Pull image 'registry.docker.nigeriainternal.com/kepay/storm:1.0.0-SNAPSHOT-$upgrade_id' failed."
  exit 1
fi
...
contrainer_ids=$(docker ps --filter name=storm -q)
if [ -n "$contrainer_ids" ]; then
  print_info "Stop container ..."
  docker stop $contrainer_ids
fi

contrainer_ids=$(docker ps --filter name=storm -aq)
if [ -n "$contrainer_ids" ]; then
...
docker run -d \
  ...
  registry.docker.nigeriainternal.com/kepay/storm:1.0.0-SNAPSHOT-$upgrade_id
